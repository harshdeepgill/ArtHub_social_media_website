...
const Navbar = () => {
    ...
    return <Box w={"100%"} bgColor={theme === "dark" ? "#15191E" : "#dddddd"} color={theme === "dark" ? "white" : "black"}>
        <Stack w={"90%"} m={"auto"} direction={"row"} display={"flex"} justifyContent={"space-between"} alignItems={"center"} p={"1"}>
            ...
            <Box display={"flex"} justifyContent={"space-between"} w={"23%"} alignItems={"center"}>
                <Icon as={theme === "dark" ? BsSun : BsMoonStars} fontSize={"xl"} onClick={changeTheme} />
                <Button bgColor="#FF7F50" color={"white"} _hover={{ backgroundColor: "tomato" }}>
                    ...
                    isAuth ?
                        (
                            <HStack spacing="1rem">
                                ...
                                <Text>{username}</Text>
                                <Button bgColor="#FF7F50" color="white" _hover={{ backgroundColor: "tomato" }} onClick={handleLogout}>Logout</Button>
                            </HStack>
