    ...
    return <Box w={"100%"} bgColor={theme === "dark" ? "#15191E" : "#dddddd"} color={theme === "dark" ? "white" : "black"}>
        <Stack w={"90%"} m={"auto"} direction={"row"} display={"flex"} justifyContent={"space-between"} alignItems={"center"} p={"1"}>
            ...
            <Box display={"flex"} justifyContent={"space-between"} w={"16%"} alignItems={"center"}>
                ...
                {
                    isAuth ? (<Button bgColor="#FF7F50" color={"white"} onClick={handleLogout}>
                        <ChakraLink as={RouteLink} style={{ textDecoration: "none", fontSize: "xl" }}>Logout</ChakraLink>
                    </Button>) : (<ChakraLink to="/login" as={RouteLink} style={{ textDecoration: "none" }} _hover={{ color: "#8FDBA7" }}>Login</ChakraLink>)
                }
            </Box>
        </Stack>
    </Box>
