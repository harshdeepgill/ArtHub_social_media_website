...
const Login = () => {
  ...
  const handleLogin = () => {
    ...
    axios.post("https://gifted-kit-cow.cyclic.app/users/login", payload)
      .then((res) => {
        ...
        login(dispatch, { username: res.data.username, avatar: res.data.avatar, subscription: res.data.subscription });
      })
  ...
  return (
    <section style={{ marginTop: "2rem", fontSize: "1.3rem" }} className={`${styles.login_form_section} ${styles.Nunito}`}>
      <p
        ref={errRef}
