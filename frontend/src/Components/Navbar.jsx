...
const Navbar = () => {
    ...
    return <Box w={"100%"} bgColor={theme == "dark" ? "#424242" : "#dddddd"} color={theme == "dark" ? "white" : "black"}>
        <Stack w={"90%"} m={"auto"} direction={"row"} display={"flex"} justifyContent={"space-between"} alignItems={"center"} p={"1"}>
            ...
            <InputGroup w={"30%"}>
                <Input value={search} placeholder="Search" focusBorderColor="coral" borderColor={"#FF7F50"} _hover={{ borderColor: "none" }} onChange={(e) => setSearch(e.target.value)} />
                <InputRightElement >
